...
```
The recall_location function provides a list of all food recalls for a designated city. Additional functionality includes the ability to specify inputs such as the firm responsible for each recall, the current status of the recall, and the distribution pattern (if known). For example, the above code will display all recalls for the city of Milwaukee from the company Tyson Foods, which has experienced several recent recalls of their chicken products due to listeria bacteria contamination.
```{r}
...
For instance, the above example displays all food recalls for Tyson Foods
