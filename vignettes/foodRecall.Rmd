...
# recall_date()
 ...
 #                                recall_initiation_date = "2022-02-01", termination_date = "2023-05-07", recalling_firm = "Garden-Fresh Foods, Inc.")
```
...
```{r baseball, echo=FALSE, fig.cap="Example of shiny app user interface", out.width = '50%'}
knitr::include_graphics("Rplot04.png")

```
Now, if we want to look at recall data with the city specified as Houston, the state as Texas, and only completed recalls to be displayed, we end up with the following for the table tab:

```{r loo, echo=FALSE, fig.cap="Example of shiny app tabular data for Houston, Texas", out.width = '90%'}
...
```{r hello, echo=FALSE, fig.cap="Map of Houston, Texas completed recalls", out.width = '50%'}
knitr::include_graphics("Rplot06.png")

```

The date tab allows one to input a recall initiation date and firm of choice, along with a product description if that is desired. Below is an example of completed recalls from Land O' Lakes initiating on May 4th, 2022.

```{r help, echo=FALSE, fig.cap="Land O' Lakes Completed Recalls initiating on May 4th, 2022", out.width = '70%'}
